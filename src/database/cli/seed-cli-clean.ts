...
async function main() {
    ...
    try {
        ...
        await dbManager.initialize(environment);
        ...
        const configManager = dbManager.getConfigurationManager();
        ...
        console.log(`  Detected as test environment: ${config.isTestEnvironment}`);
        console.log(`  Using TEST_DATABASE_URL: ${config.usingTestDatabaseUrl}`);

        // Validate database before operations
        const validator = new DatabaseValidator(config.databaseUrl);

        console.log('\n=== Database Validation ===');
        const connectionValid = await validator.validateConnection(config.databaseUrl);
        if (!connectionValid) {
            throw new Error('Database connection validation failed');
        }
        console.log('✓ Database connection validated');

        const environmentValid = await validator.validateEnvironment(environment);
        if (!environmentValid) {
