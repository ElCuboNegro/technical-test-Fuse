...
import { EventCollector, DEFAULT_EVENT_COLLECTOR_CONFIG } from '../../src/conversation-logging/collectors/event-collector';
...
class MockPseudonymizationEngine implements PseudonymizationEngine {
    ...
    async maskSSN(): Promise<{ masked: string; hash: string }> {
        return { masked: '****', hash: 'hash123' };
    }

    async maskDOB(): Promise<{ masked: string; hash: string }> {
        return { masked: '****-**-**', hash: 'hash456' };
    }

    async maskEmail(): Promise<{ masked: string; domain?: string }> {
        return { masked: '****@****.***' };
    }
...
describe('EventCollector', () => {
    ...
    describe('Non-blocking event acceptance', () => {
        it('should accept events for all EventType values without blocking', async () => {
            ...
            const acceptPromises = events.map(event => eventCollector.acceptEvent(event));
    ...
    describe('Duplicate event detection and handling', () => {
        ...
        it('should use session_id, thread_id, step_index, and event_type for duplicate detection', async () => {
            ...
            await eventCollector.acceptEvent({ ...baseEvent, session_id: 'session_2' });
            ...
            await eventCollector.acceptEvent({ ...baseEvent, event_type: EventType.MESSAGE_AGENT });
    ...
    describe('Statistics and monitoring', () => {
        ...
        it('should track active operations count', async () => {
            ...
            const slowQueue = {
                ...mockQueue,
                enqueue: (event: EventEnvelope) => new Promise(resolve =>
                    setTimeout(() => {
    ...
    describe('Error handling and resilience', () => {
        ...
        it('should continue processing other events when one fails', async () => {
            ...
            const originalPseudonymize = mockEngine.pseudonymizePayload.bind(mockEngine);
            ...
            await Promise.all(events.map(event => eventCollector.acceptEvent(event)));
            await eventCollector.waitForActiveOperations();
